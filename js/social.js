...
export function updateQuests(game, dt) {
  ...
  if (threats.length >= 3 || game.war?.atWar) {
    offerQuest(game, {
      ...
      giverId: giver.id,
      need: { kills: Math.min(5, threats.length) },
      progress: { kills: 0 },
...
function tryCompleteQuest(game, q) {
  if (q.type === "food") {
    q.progress.food = game.stock.food;
    if (game.stock.food >= q.need.food) completeQuest(game, q);
  } else if (q.type === "wood") {
    q.progress.wood = game.stock.wood;
    if (game.stock.wood >= q.need.wood) completeQuest(game, q);
...
/** Find cover tile (tree adjacent / wall adjacent) away from threat */
  ...
  for (let y = oy - 6; y <= oy + 6; y++) {
    for (let x = ox - 6; x <= ox + 6; x++) {
      ...
      for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
        ...
        if (game.world.buildings[sy][sx]?.done && game.world.buildings[sy][sx].type !== "gate") continue;
        if (game.world.fire[sy][sx] > 0.2) continue;
        const away = Math.hypot(sx - threatX, sy - threatY);
        const near = Math.hypot(sx - s.x, sy - s.y);
        const score = away * 1.4 - near * 0.5 + (tree ? 1 : 2);
        if (score > bestScore) {
          bestScore = score;
...
export function assessThreat(s, threat, allies, game) {
  ...
  const allyPower = allies.length * 16 + (s.life?.groupId ? 12 : 0);
  const myPower = bravery * 40 + wpn.dmg * 18 + (s.military ? 20 : 0) + (rep > 0.3 ? 8 : 0);
  const danger = (threat.unit.damage || 12) + threat.unit.hp * 0.12 + (threat.dist < 2 ? 15 : 0);
  const fightScore = myPower + allyPower - danger - fear * 35;
  const fleeScore = 20 + fear * 40 + (5 - Math.min(5, threat.dist)) * 10 - bravery * 25;
  const coverScore = 25 + (wpn.style === "ranged" ? 30 : 10) + fear * 15;
  const callScore = allies.length < 2 && fear > 0.25 ? 35 + (1 - bravery) * 30 : 10;
...
export function activeQuestSummary(game) {
  ...
  if (q.type === "food") return `${q.title}: ${Math.floor(game.stock.food)}/${q.need.food}`;
  if (q.type === "wood") return `${q.title}: ${Math.floor(game.stock.wood)}/${q.need.wood}`;
  return q.title;
}
